...
For NixOS, import [`nix/module.nix`](nix/module.nix) in the system configuration, then rebuild. Ubuntu/Fedora users should install `adb`, `scrcpy`, PipeWire tools and the distribution package named `v4l2loopback`; load it with `exclusive_caps=1`, `video_nr=10`, and `card_label="Android Hub Camera"`.
